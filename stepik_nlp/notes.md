...
## 3.4 Свёрточные нейросети для обработки текстов
Свертка:
* выделение локальных особенностей сигнала (паттернов) вне зависимости от позиции
* описание искомого паттерна - ядро свертки

Применение сверток к тексту: 
* строится матрица эмбеддингов слов или символов `(sentence_len x embedding_dim)`
* к полученной матрице применяется банк фильтров одномерных сверток (размер каждого ядра `k x embedding_dim`)

Для расширения пятна восприятия (receptive field) (возможности обрабатывать широкий контекст) применяют pooling: матрицу эмбеддингов агрегируют по строкам

Особенности:
* хорошо обучаются и сходятся
* нет проблемы затухающего градиента с ростом длины входной последовательности, что свойственно RNN
* в свёрточных нейросетях проблема затухания градиента проявляется с увеличением глубины (то есть количества слоёв). Самые распространённые методы борьбы с затуханием градиента:
    * частично-линейные функции активации (например, LeakyReLU)
    * блоки со связями в обход нелинейностей (skip connections, residual blocks)
    * нормализация (например, BatchNorm)
* подходят для распознавания локальных паттернов, словосочетаний, коротких фраз
* слабо подходят для учета широкого контекста
* могут учитывать более широкий контекст (receptive field) без увеличения числа параметров
    * применяются разреженные свертки или dilated convolutions
    * шаг прореживания увеличивается с глубиной: первый слой без прореживания, а на каждом следующем слое шаг увеличивается в 2 раза 

способ применения сверток для генерации текста - авторегрессионные модели
